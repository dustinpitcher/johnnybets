...
Transactional emails (password resets, invite notifications) use Azure Communication Services.

| Component | Value |
|-----------|-------|
| Service | `acs-jbet-prod-eus2` |
| Sender | `noreply@johnnybets.ai` |
| Library | `@azure/communication-email` |
| Code | `web/lib/email.ts` |

### Email Functions
- `sendPasswordResetEmail(email, token)` - Password reset flow
- `sendInviteNotification(email, inviteCode)` - Waitlist invite emails
- `sendGenericEmail(to[], subject, html)` - Future marketing use

### User Groups (for email targeting)
Users can belong to multiple groups via `UserGroupMembership` model. Groups are used for targeted email campaigns.
...
- `AZURE_COMMUNICATION_CONNECTION_STRING` - Email service (ACS)
...
| Communication Services | `acs-jbet-prod-eus2` | (shared) |
| Email Service | `es-jbet-prod-eus2` | (shared) |
